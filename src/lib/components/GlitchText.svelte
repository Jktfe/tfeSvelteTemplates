<!--
  ...
  WHAT — Wraps text in a real DOM node plus two CSS pseudo-element
  ...
  DEPENDENCIES — Zero external dependencies. Pure CSS pseudo-elements and a
  small rAF/timeout loop for the deterministic jitter.

  PERFORMANCE — One rAF loop per active instance updates four offsets
  (quantised to 80ms steps) plus an occasional tear timeout. The loop
  only runs while the effect is active and is cancelled on destroy.
