...
<script lang="ts">
	...
	// Svelte's fly/fade are JS-driven, so a CSS media query alone can't stop
	// them. We mirror the OS preference here and zero the durations instead.
...
>
	{#each displayedToasts as toast (toast.id)}
		<div
			...
			in:fly={{ y: position.startsWith('top') ? -20 : 20, duration: reducedMotion ? 0 : 300 }}
