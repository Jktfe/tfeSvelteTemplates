...
describe('componentCatalog agent metadata', () => {
	...
	it('points every entry at a real WebP screenshot in static/ComponentScreenshots', () => {
		// Hand-drawn SVG placeholders slipped into the catalogue once, and every
		// capture has since been converted to WebP — real WebP captures only.
		const problems = componentCatalogEntries.flatMap(({ item }) => {
			...
			if (!/\.webp$/i.test(item.screenshot)) {
				issues.push(`${item.name}: screenshot must be a WebP capture, not ${item.screenshot}`);
			}
